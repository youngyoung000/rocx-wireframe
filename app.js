(function () {
  ...
  function renderLeaderboard(route) {
    ...
    return `
      ...
      <div class="leader-podium">
        ${rows.slice(0,3).map((row, index) => `<article class="panel podium-card ${index === 0 ? "first" : index === 1 ? "second" : "third"}"><span class="rank-medal">${row.rank}</span><h3>${row.label}</h3><p>${row.user}</p><strong>${isPoa ? `Net Score ${row.netScore}` : row.value}</strong>${isPoa ? `<span class="podium-reward">Est. AE Reward · ${row.reward}</span>` : ""}</article>`).join("")}
      </div>
      <div class="leader-layout">
        <section class="panel table-wrap"><table class="data-table"><thead>${isPoa ? `<tr><th>Rank</th><th>Contributor</th><th>Helpful</th><th>Not Helpful</th><th>Net Score</th><th style="text-align:right">Est. AE Reward</th></tr>` : `<tr><th>Rank</th><th>User</th><th style="text-align:right">Total AE</th></tr>`}</thead><tbody>${rows.map(row => isPoa
  ...
  function renderMyOverview() {
    return `<div class="metric-banner compact"><div class="metric"><small>My AE</small><strong>12,450 AE</strong></div><div class="metric"><small>Leaderboard AE</small><strong>4,820 AE</strong></div><div class="metric"><small>Streak</small><strong>18 days</strong></div></div>
